...
class MEMThought:
    ...
    # New tick-based analysis
    tick_analysis: Optional[Dict[str, Any]] = None
from pathlib import Path
...
class MemGPTTradingViewCompanion:
    ...
    def _generate_smart_decision(self, symbol: str) -> MemGPTDecision:
        """Generate realistic decision based on market analysis with tick-level insights"""
        try:
            ...
            trades_response = requests.get(f'https://api.binance.com/api/v3/aggTrades?symbol={symbol}&limit=1000', timeout=2)
            ...
            if stats_response.status_code == 200:
                ...
                # Tick Analysis
                tick_analysis = self._analyze_tick_data(trades_response.json() if trades_response.status_code == 200 else [])
                
                # Enhanced decision making with tick data
                base_confidence = 0.5
                tick_boost = 0.0
                
                if tick_analysis['volume_surge'] and tick_analysis['price_acceleration']:
                    tick_boost = 0.25
                    reasoning_prefix = "🎯 1000-TICK SURGE: "
                elif tick_analysis['institutional_flow']:
                    tick_boost = 0.15
                    reasoning_prefix = "🏦 INSTITUTIONAL TICK FLOW: "
                else:
                    reasoning_prefix = ""
                
                # Generate realistic MemGPT-style decision with tick enhancement
                if price_change_pct > 5:
                    action = "sell"
                    confidence = min(0.95, 0.8 + tick_boost)
                    reasoning = f"{reasoning_prefix}Strong upward momentum (+{price_change_pct:.1f}%), profit-taking opportunity"
                    risk = "medium" if not tick_analysis['volume_surge'] else "low"
                elif price_change_pct < -5:
                    action = "buy"
                    confidence = min(0.95, 0.75 + tick_boost)
                    reasoning = f"{reasoning_prefix}Oversold conditions ({price_change_pct:.1f}%), potential bounce"
                    risk = "medium" if not tick_analysis['volume_surge'] else "low"
                elif abs(price_change_pct) > 2:
                    action = "analyze"
                    confidence = min(0.85, 0.6 + tick_boost)
                    reasoning = f"{reasoning_prefix}Moderate movement ({price_change_pct:.1f}%), monitoring for confirmation"
                    risk = "low"
                else:
                    action = "hold"
                    confidence = max(0.2, 0.4 - (tick_boost * 0.5))  # Lower confidence in sideways with tick activity
                    reasoning = f"{reasoning_prefix}Sideways movement ({price_change_pct:.1f}%), waiting for clear signal"
                    risk = "low"
    ...
    def _analyze_tick_data(self, trades_data: List) -> Dict[str, Any]:
        """Analyze tick-level trading data for enhanced signals"""
        ...
        try:
            # Analyze last 1000 ticks/trades
            total_volume = sum(float(trade['q']) for trade in trades_data)
            total_trades = len(trades_data)
            avg_trade_size = total_volume / total_trades if total_trades > 0 else 0
            
            # Price movement analysis
            prices = [float(trade['p']) for trade in trades_data]
            price_changes = [prices[i] - prices[i-1] for i in range(1, len(prices))]
            
            # Volume surge detection
            recent_volume = sum(float(trade['q']) for trade in trades_data[-200:])  # Last 200 trades
            earlier_volume = sum(float(trade['q']) for trade in trades_data[-400:-200])  # Previous 200 trades
            volume_surge = recent_volume > earlier_volume * 1.5 if earlier_volume > 0 else False
            
            # Price acceleration
            recent_price_changes = price_changes[-100:] if len(price_changes) >= 100 else price_changes
            price_acceleration = abs(sum(recent_price_changes)) > abs(sum(price_changes[:-100])) * 1.3 if len(price_changes) > 100 else False
            
            # Large trades (institutional flow)
            large_trades = [trade for trade in trades_data if float(trade['q']) > avg_trade_size * 3]
            large_trades_ratio = len(large_trades) / total_trades if total_trades > 0 else 0
            institutional_flow = large_trades_ratio > 0.15  # >15% large trades
            
            # Tick momentum
            net_price_change = sum(price_changes) if price_changes else 0
            if net_price_change > avg_trade_size * 0.001:  # Significant positive movement
                tick_momentum = 'strong_up'
            elif net_price_change < -avg_trade_size * 0.001:  # Significant negative movement
                tick_momentum = 'strong_down'
            elif abs(net_price_change) > avg_trade_size * 0.0005:  # Moderate movement
                tick_momentum = 'moderate_up' if net_price_change > 0 else 'moderate_down'
            else:
                tick_momentum = 'neutral'
            ...
            return {
                ...
                'optimal_timeframe': self._calculate_optimal_timeframe(volume_surge, price_acceleration, net_price_change, avg_trade_size)
            }
            
        except Exception as e:
            logger.debug(f"Tick analysis error: {e}")
            return {
                'volume_surge': False,
                'price_acceleration': False,
                'institutional_flow': False,
    ...
    def _calculate_optimal_timeframe(self, volume_surge: bool, price_acceleration: bool, price_change: float, avg_trade_size: float) -> Dict[str, Any]:
        ...
        volatility_level = "normal"
        if abs(price_change) > avg_trade_size * 0.002:  # High price movement
            volatility_level = "high"
        elif abs(price_change) > avg_trade_size * 0.001:  # Moderate movement
            volatility_level = "elevated"
        elif abs(price_change) < avg_trade_size * 0.0005:  # Low movement
            volatility_level = "low"
        
        # Volume assessment
        volume_level = "high" if volume_surge else "normal"
        
        # Market regime detection
        if volatility_level == "high" and volume_level == "high":
            regime = "breakout"
            optimal_minutes = max(1, base_timeframe // 3)  # Ultra-short for breakouts
            category = "SCALPING"
        elif volatility_level == "high" or volume_level == "high":
            regime = "trending"
            optimal_minutes = max(2, base_timeframe // 2)  # Short for trending
            category = "SHORT-TERM"
        elif volatility_level == "low" and volume_level == "normal":
            regime = "consolidation"
            optimal_minutes = base_timeframe * 2  # Longer for consolidation
            category = "MEDIUM-TERM"
        else:
            regime = "ranging"
            optimal_minutes = base_timeframe
            category = "SHORT-TERM"
        
        # Confidence boost based on regime
        confidence_boost = {
            "breakout": 0.15,
            "trending": 0.10, 
            "ranging": 0.0,
