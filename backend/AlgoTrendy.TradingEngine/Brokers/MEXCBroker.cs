...
{
    private MexcRestClient? _client;
    ...
    /// </summary>
    private MexcRestClient GetClient()
    {
        if (_client == null)
        {
            _client = new MexcRestClient(opts =>
            {
    ...
    public async Task<Order> PlaceOrderAsync(OrderRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = request.Type switch
            {
                ...
                Core.Enums.OrderType.StopLoss => await GetClient().SpotApi.Trading.PlaceOrderAsync(
                    symbol: request.Symbol,
                    side: orderSide,
                    type: orderType,
                    quantity: request.Quantity,
            ...
            var order = new Order
            {
                ...
                ExchangeOrderId = mexcOrder.OrderId,
                Symbol = request.Symbol,
                Exchange = BrokerName,
                Side = request.Side,
                Type = request.Type,
                ...
                FilledQuantity = mexcOrder.QuantityFilled,
                Price = request.Price,
    ...
    public async Task<Order> CancelOrderAsync(string orderId, string symbol, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var order = new Order
            {
                ...
                Type = MapOrderTypeFromMEXC(mexcOrder.OrderType),
    ...
    private static Mexc.Net.Enums.OrderType MapOrderType(Core.Enums.OrderType type)
    {
        return type switch
        {
            Core.Enums.OrderType.Market => Mexc.Net.Enums.OrderType.Market,
            Core.Enums.OrderType.Limit => Mexc.Net.Enums.OrderType.Limit,
            Core.Enums.OrderType.StopLoss => Mexc.Net.Enums.OrderType.LimitMaker,
            Core.Enums.OrderType.StopLimit => Mexc.Net.Enums.OrderType.LimitMaker,
            Core.Enums.OrderType.TakeProfit => Mexc.Net.Enums.OrderType.TpSlOrder,
            _ => throw new ArgumentException($"Invalid order type: {type}")
        };
    }

    private static Core.Enums.OrderType MapOrderTypeFromMEXC(Mexc.Net.Enums.OrderType type)
    {
        return type switch
        {
            Mexc.Net.Enums.OrderType.Market => Core.Enums.OrderType.Market,
            Mexc.Net.Enums.OrderType.Limit => Core.Enums.OrderType.Limit,
            Mexc.Net.Enums.OrderType.LimitMaker => Core.Enums.OrderType.StopLoss,
            Mexc.Net.Enums.OrderType.TpSlOrder => Core.Enums.OrderType.TakeProfit,
            Mexc.Net.Enums.OrderType.ImmediateOrCancel => Core.Enums.OrderType.Market,
            Mexc.Net.Enums.OrderType.FillOrKill => Core.Enums.OrderType.Market,
    ...
    private static Core.Enums.OrderStatus MapOrderStatus(Mexc.Net.Enums.OrderStatus status)
    {
        return status switch
        {
            ...
            Mexc.Net.Enums.OrderStatus.PartiallyCanceled => Core.Enums.OrderStatus.Cancelled,
